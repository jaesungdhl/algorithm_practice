...
function kira(a, b, c) {
    ...
    for(let i = 0 ; i < a.length ; i++) {
      remainderArr.push(a[i] % b[i]);
    }
    let biggestRemainder = remainderArr.sort((a,b) => b - a);
    return biggestRemainder[0] % 2 === 0 ? c.toLowerCase() : c.toUpperCase();
  }

  const kira = (a, b, c) => a.reduce((s,n,i)=> Math.max(s, a[i]%b[i]),0)%2 === 1  ? c.toUpperCase() : c.toLowerCase();