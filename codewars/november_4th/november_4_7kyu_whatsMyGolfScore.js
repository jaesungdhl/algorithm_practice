...
function golfScoreCalculator(parList, scoreList){
    let parListArr = parList.split("");
    let scoreListArr = scoreList.split("");
    let calculatedScoreArr = [];
    for(let i = 0 ; i < parListArr.length ; i++) {
      calculatedScoreArr.push( Number(scoreListArr[i]) - Number(parListArr[i]) );
    }
    return calculatedScoreArr.reduce((a,n) => a += n);
  }
