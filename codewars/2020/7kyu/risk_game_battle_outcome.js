...
function battleOutcome( attacker , defender ) {
    ...
    let UnitsLost = [0,0], 
        lengthToLoop = defender.length > attacker.length ? attacker.length : defender.length;
  ...
  function battleOutcome( attacker , defender ) {
    let atk_arr = [...attacker];
    let def_arr = [...defender];
    let atk_death = 0;
    let def_death = 0;
    
    //parse based on smaller number of dice played
    let index = Math.min(atk_arr.length, def_arr.length);
    
    for(let i = 0; i < index; i++)
    {
      //get max from attacker and defender
      let atk = Math.max(...atk_arr);
      let def = Math.max(...def_arr);
      //get max value index from attacker and defender
      let atk_index = atk_arr.indexOf(atk);
      let def_index = def_arr.indexOf(def);
      //remove data from array
      atk_arr.splice(atk_index, 1);
      def_arr.splice(def_index, 1);
      //add death toll to corresponding var
      if(atk > def)
        def_death += 1;
      else
        atk_death += 1;
