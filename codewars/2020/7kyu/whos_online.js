...
const whosOnline = (friends) => {
    ...
    friends.forEach(friend => {
      if(friend.status === "offline") status.offline.push(friend.username);
      else {
        if(friend.lastActivity > 10) status.away.push(friend.username);
        else status.online.push(friend.username);
      }
    })
