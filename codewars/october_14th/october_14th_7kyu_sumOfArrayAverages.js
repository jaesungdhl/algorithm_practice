...
const sumAverage = (arr) => {
    let sumArr = arr.map(subArr => subArr.reduce((a,n) => a += n));
    let subArrLength = arr.map(subArr => subArr.length);
    sumArr = sumArr.map((num, index) => num / subArrLength[index]);
    return Math.floor(sumArr.reduce((a,n) => a += n));
  }
