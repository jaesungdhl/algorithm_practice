...
Find more details below:
  ...
  function checkDigit(number, index1, index2, digit) {
    if(index1 > index2) return number.toString().slice(index2, index1 + 1).includes(digit.toString());
    else return number.toString().slice(index1, index2 + 1).includes(digit.toString());
  };

  const checkDigit = (n, x, y, d) => n.toString().slice(Math.min(x, y), Math.max(x, y) + 1).indexOf(d) > -1;